...
- 재생 버튼을 누르면 현재 행동값을 반복 적용해 시뮬레이션이 자동으로 계속 진행된다.
...
- `재생`은 한 번의 API 요청으로 여러 물리 프레임을 건너뛰지 않고, 공식 wrapper의 한 제어 스텝을 반복 호출한다.
- 수동 모드에서는 마지막으로 선택된 `[steer, gas, brake]`를 다음 스텝에도 유지한다.
- 재생 속도는 브라우저의 호출 간격만 조정하며, 각 호출의 물리 규칙은 동일하게 유지한다.
- 완주, 리타이어, 시간 초과, 서버 오류가 발생하면 자동으로 재생을 멈춘다.
- 동시에 두 개의 step 요청이 보내지지 않도록 클라이언트에 in-flight 잠금을 둔다.
- 이후 Agent 모드가 추가되면 마지막 행동값 대신 매 스텝 `Agent.act(observation)`의 결과를 사용한다.
...
- 재생 버튼만으로 현재 행동을 반복해 차량이 자동으로 계속 움직인다.
- 완주나 리타이어 시 자동 재생이 안전하게 멈춘다.

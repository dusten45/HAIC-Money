...
def build_env(track_id, seed, max_steps, frame_skip, reward_shaping, obstacles=True):
        ...
        for i in range(n_envs):
            ...
            if training_track_mode == "sampled":
                ...
                yield lambda tid=track_id, sd=seed: build_env(
                    tid, sd, max_steps, frame_skip, reward_shaping, obstacles
                )
...
def evaluate(model, track_id, seed, max_steps, frame_skip, episodes=1):
    ...
    for _ in range(episodes):
        env = build_env(
            track_id, seed, max_steps, frame_skip, reward_shaping=False, obstacles=True
        )
...
class HoldoutEvaluator:
    ...
    def evaluate(self, model, is_final=False):
        ...
        if is_best:
            ...
            cpu_best = PPO.load(str(self.run_dir / f"{BEST_MODEL_FILENAME}.zip"), device="cpu")

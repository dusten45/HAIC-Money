...
초기화된 racing-line safety controller가 도로 중심선·도로 폭·밝은 위험 영역 신호를 사용해
급격한 역주행성 조향을 제한합니다. 회색 변환 뒤 구분되지 않는 초록 배경과 장애물은 모두
도로 내부의 위험 침입으로 취급하고, 도로 폭이 좁아지거나 중심 차량의 가장자리 여유가
사라지면 초록색 접촉 위험으로 우선 분류해 제동한 뒤 제한된 저속 throttle로 복귀합니다.
중심 픽셀이 아직 아스팔트에 있어도 차량 가장자리가 초록색에 닿을 여유가 부족하면
먼저 차단합니다. 시야가 잠시 끊겨도 같은
유한 복구 단계를 사용하므로 안전 필터가 영구 정지 명령으로 남지 않습니다. 직선 구간은 중심선 deadband 안에서 조향을 0으로
고정하고, 곡선 구간도 최대 조향량과 프레임별 조향 변화량을 제한합니다. 가속과 제동은
상호 배타적으로 선택하며, 초기화된 racing-line controller는 안전한 직선 통로에서의
기본 순항 목표와 가속 상한을 각각 `72`와 `0.18`로 설정합니다. 작은 픽셀 양자화로
생기는 회전 전 흔들림은 직선 deadband에
흡수하고, 중심선 오차와 방향 변화가 서로 반대일 때 방향 변화 항을 억제해 반대
조향을 만들지 않습니다. 조향 부호를 바꿀 때는 항상 0을 거치므로 속도 향상과
드리프트 억제를 분리합니다. 커브는 먼 거리 중심선과 중간 중심선의 변화를
미리 읽어 목표 속도를 먼저 낮추며, 곡률이 커질수록 throttle도 연속적으로
낮춰 트랙 안에서 조향할 여유를 확보합니다. 차체의 조향 반응이 큰 점을
고려해 커브에서는 steering gain도 감쇠하고, 목표 속도보다 빠를 때는 추가로
감쇠하여 작은 조향 입력이 큰 회전으로 증폭되는 것을 억제합니다.
장애물이 먼 위치에서 감지되어도 gas는 즉시 차단해 회피 공간을 확보합니다.
근거리 도로가 사라지면 마지막 near/far 중심선과 진행 방향 오차를 이용해 도로 쪽으로
복귀 조향을 생성하고, 화면 전체 재검색으로 차량이 중심선 탐색 창을 벗어난 경우도
복구합니다. 복구 조향은 정상 조향과 동일하게 프레임별 변화량을 제한합니다.
이 런타임은 과거의 특정 곡선/잔디/속도 우선순위를 누적하는 방식이 아니라, 실제
자율주행 스택처럼 `인식 → 주행 가능 영역 → 경로 추종 → 속도 계획 → 안전 제약`의
단일 계층형 제어로 정리되어 있습니다. 장애물은 화면상의 거리를 TTC 근사값으로
사용해 가까워질수록 속도 목표를 연속적으로 낮추고, 회피 방향은 중심선의 좌우가
아니라 보이는 도로 폭이 더 넓은 쪽을 선택합니다. 명시적 action-contract payload와
DrQ actor는 기록된 모델 계약을 그대로 유지합니다. CEM은 별도 학습 모델과 평가 결과가
선택한 경우에만 PPO 행동 제안을 보정합니다.
F1의 트랙 리밋 원칙을 모방해 near-row 도로 폭에서 차체 여유를 뺀 안전 구간을
계산하고, 모든 곡률·장애물 조향 제안을 그 구간 안으로 제한합니다. 직선에서는
가속을 허용하고, 커브 진입 전에는 직선 제동으로 목표 속도를 낮춘 뒤 apex 부근에서
조향을 유지하며 출구에서 다시 가속하도록 구성했습니다.
직선에서 먼 장애물이 보이면 장애물 반대편의 더 넓은 안전 가장자리로 미리 이동하며,
급커브 preview가 감지되면 진입 속도 하한을 더 낮추고 조향 변화량을 줄입니다. 최종
조향 명령에도 외향 성분 차단을 적용해 차량 중심이 트랙 밖을 향하지 않도록 합니다.
장애물 인식은 고정된 작은 스프라이트 크기에 의존하지 않고, 도로 중심 주변의 다양한
크기와 종횡비를 가진 밝은 연결 영역을 주행 위험으로 취급합니다.

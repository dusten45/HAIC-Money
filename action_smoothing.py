...
_SCHEMA_VERSION = 1
CHECKPOINT_ACTION_SMOOTHING_MEMBER = "haic_action_smoothing.json"
...
DEFAULT_ACTION_CONTROL = {"method": "none"}
...
def canonical_action_smoothing(
    ...
    return {
        ...
        "low": list(ACTION_BOUNDS_LOW),
        "high": list(ACTION_BOUNDS_HIGH),
        "update_unit": "policy_action",
        "clip_order": "before_and_after",
    }
...
def normalize_action_smoothing(config: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    for key, expected in expected_metadata.items():
        if key in config and config[key] != expected:
            raise ValueError(f"action smoothing {key} does not match schema")
...
def canonical_action_control(method: str = "none") -> dict[str, Any]:
    if not isinstance(method, str):
        raise TypeError("action control method must be a string")
    method = method.strip().lower()
    aliases = {
        "previous-steering-plane": "previous_applied_steering_plane",
        "constant-plane": "constant_steering_plane",
    }
    method = aliases.get(method, method)
    if method not in {
        "none", "constant_steering_plane", "previous_applied_steering_plane",
    }:
        raise ValueError("unsupported action control method")
    if method == "none":
        return {"schema_version": 1, "method": "none", "input_channels": 4}
    return {
        "schema_version": 1,
        "method": method,
        "input_channels": 5,
        "plane_index": 4,
        "encoding": "affine_-1_1_to_0_1",
        "timing": "before_policy_action",
        "constant_value": 0.5 if method == "constant_steering_plane" else None,
    }


def normalize_action_control(config: Mapping[str, Any] | None = None) -> dict[str, Any]:
    if config is None:
        config = DEFAULT_ACTION_CONTROL
    if not isinstance(config, Mapping):
        raise TypeError("action control config must be a mapping")
    normalized_config = dict(config)
    normalized_config["method"] = {
        "previous-steering-plane": "previous_applied_steering_plane",
        "constant-plane": "constant_steering_plane",
    }.get(normalized_config.get("method", "none"), normalized_config.get("method", "none"))
    canonical = canonical_action_control(normalized_config["method"])
    for key, value in canonical.items():
        if key in normalized_config and normalized_config[key] != value:
            raise ValueError(f"action control {key} does not match schema")
    return canonical


def action_control_fingerprint(config: Mapping[str, Any] | None = None) -> str:
    encoded = json.dumps(
        normalize_action_control(config),
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=True,
        allow_nan=False,
...
def validate_action_control(action_smoothing, action_control) -> None:
    smoothing = normalize_action_smoothing(action_smoothing)
    control = normalize_action_control(action_control)
    if control["method"] != "previous_applied_steering_plane":
        return
    if smoothing["method"] != "alpha":
        raise ValueError("previous steering control requires alpha smoothing")
    alpha = smoothing["alpha"]
    if not isinstance(alpha, list) or len(alpha) != 3:
        raise ValueError("previous steering control requires per-action alpha")
    if not 0.0 < alpha[0] < 1.0 or alpha[1:] != [1.0, 1.0]:
        raise ValueError("previous steering control requires steering-only smoothing")


def append_action_control_plane(observation, action_control, previous_action) -> np.ndarray:
    control = normalize_action_control(action_control)
    values = np.asarray(observation, dtype=np.float32)
    if control["method"] == "none":
        return values
    if values.ndim != 3 or values.shape[0] != 4:
        raise ValueError("action control expects a 4-channel single observation")
    if control["method"] == "constant_steering_plane":
        encoded = 0.5
    else:
        if previous_action is None or len(previous_action) != 3:
            raise ValueError("previous steering action is required")
        encoded = (float(previous_action[0]) + 1.0) / 2.0
    if not 0.0 <= encoded <= 1.0:
        raise ValueError("encoded steering must be within [-1, 1]")
    plane = np.full((1, values.shape[1], values.shape[2]), encoded, dtype=np.float32)
    return np.concatenate((values, plane), axis=0)
...
def read_embedded_action_smoothing(model_path) -> dict[str, Any] | None:
    """Read and validate smoothing provenance embedded in an SB3 zip."""

    try:
        with zipfile.ZipFile(model_path) as archive:
            payload = archive.read(CHECKPOINT_ACTION_SMOOTHING_MEMBER)
    except (FileNotFoundError, OSError, KeyError, zipfile.BadZipFile):
        return None
    recorded = json.loads(payload.decode("utf-8"))
    config = normalize_action_smoothing(recorded.get("config"))
    recorded_fingerprint = recorded.get("fingerprint")
    if (
        recorded_fingerprint is not None
        and recorded_fingerprint != action_smoothing_fingerprint(config)
    ):
        raise ValueError("embedded action smoothing fingerprint does not match config")
    return config


def write_embedded_action_smoothing(model_path, config) -> None:
    """Attach immutable smoothing provenance to a newly saved SB3 zip."""

    canonical = normalize_action_smoothing(config)
    payload = {
        "config": canonical,
        "fingerprint": action_smoothing_fingerprint(canonical),
    }
    with zipfile.ZipFile(model_path, mode="a", compression=zipfile.ZIP_DEFLATED) as archive:
        if CHECKPOINT_ACTION_SMOOTHING_MEMBER in archive.namelist():
            recorded = json.loads(
                archive.read(CHECKPOINT_ACTION_SMOOTHING_MEMBER).decode("utf-8")
            )
            existing = normalize_action_smoothing(recorded.get("config"))
            if existing != canonical:
                raise ValueError("checkpoint already contains different action smoothing")
            return
        archive.writestr(
            CHECKPOINT_ACTION_SMOOTHING_MEMBER,
